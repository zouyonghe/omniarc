...
def snapshot_debug_context(job_id: str, artifacts_dir: Path) -> dict:
    return {
        ...
        "replay": replay_run(job_id, artifacts_dir=str(artifacts_dir), start=0, limit=5),
...
def test_mcp_lifecycle_e2e(tmp_path: Path) -> None:
    ...
    replay_payload = replay_run(job_id, artifacts_dir=str(artifacts_dir), start=0, limit=10)
    root_listing = get_run_artifact(job_id, "", artifacts_dir=str(artifacts_dir))
    status_file = get_run_artifact(job_id, "status.json", artifacts_dir=str(artifacts_dir))

    assert completed["status"] == "completed", debug
    assert inspect_payload["status"]["job_id"] == job_id, debug
    assert inspect_payload["task"]["task"] == "Open Safari and search for OmniArc MCP", debug
    ...
    assert any(action["kind"] == "open_app" for action in replay_payload["actions"]), debug
